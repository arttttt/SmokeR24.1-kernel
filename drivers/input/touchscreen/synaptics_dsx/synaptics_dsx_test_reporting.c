...
struct synaptics_rmi4_f54_handle {
	...
	struct rmi_f54_self_test_data *test_data;
...
show_store_prototype(self_test)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	return (f54->status == STATUS_IDLE) ? count : f54->status;
}

static bool synaptics_rmi4_f54_test_data_check(int index)
{
	bool result = true;
	short val = 0;
	unsigned int i = 0;
	...
	standard_data = (short *)f54->test_data[index].data;
	len = f54->test_data[index].size;

	for (i = 0; i < len; i += 2) {

		val = (f54->report_data[i+1] << 8) | f54->report_data[i];
		if (standard_data[i] == 0 && standard_data[i+1] == 0) {
			continue;
		}
		min = standard_data[i];
		max = standard_data[i+1];
		if (unlikely(standard_data[i] > standard_data[i+1])) {
			max = standard_data[i];
			min = standard_data[i+1];
		}
		if (val > max || val < min) {
			dev_err(rmi4_data->pdev->dev.parent,
					"error at %d, ref data = (%d,%d), current = %d\n",
					i/2, min, max, val);
			result = false;
			break;
		}
...
static ssize_t synaptics_rmi4_f54_self_test_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	...
	const struct firmware *test_data = NULL;
	const u8 *data;

	do_preparation();

	pr_info("self_test_name = %s\n", bdata->self_test_name);
	error = request_firmware(&test_data, bdata->self_test_name, dev);
	if (error < 0) {
		dev_err(dev, "Failure to request test data %s\n",
			bdata->self_test_name);
		memcpy(f54->test_result, "255", 3);
		return error;
	}

	data = test_data->data;

	f54->test_data = kzalloc((sizeof(test_array) / sizeof(char *)) *
					sizeof(struct rmi_f54_self_test_data), GFP_KERNEL);
	if (f54->test_data == NULL) {
		error = -ENOMEM;
		goto end;
	}

	memcpy(f54->test_result, "0", strlen("0"));

	for (i = 0; i < sizeof(test_array) / sizeof (char *); i++) {
		len = data[0] | (data[1] << 8);
		pr_info("len = %d\n", len);
		f54->test_data[i].size = len;
		len *= sizeof(short);
		data = data + 2;
		f54->test_data[i].data = (short *)kmalloc(len, GFP_KERNEL);
		if (f54->test_data[i].data == NULL) {
			error = -ENOMEM;
			goto free_buf;
		}

		for (j = 0; j < len; j += 2)
			f54->test_data[i].data[j/2] = data[j] | (data[j+1] << 8);
		data += len;
		dev_info(rmi4_data->pdev->dev.parent, "test start at %d\n", i);
		error = synaptics_rmi4_f54_report_type_store(dev, NULL,
				test_array[i], strlen(test_array[i]));
		if (error <= 0)
			goto free_buf;
		error = synaptics_rmi4_f54_get_report_store(dev, NULL,
				buf, count);
		if (error <= 0)
			goto free_buf;

		result = synaptics_rmi4_f54_test_data_check(i);
		if (!result) {
			memcpy(f54->test_result, test_array[i], strlen(test_array[i]));
			break;
		}
	}
...
free_buf:
	for (i = 0; i < sizeof(test_array) / sizeof(char *); i++) {
		if (f54->test_data[i].data != NULL)
			kfree(f54->test_data[i].data);
	}
	kfree(f54->test_data);
end:
	release_firmware(test_data);
	if (error == 0)
		return count;
	else
		return error;
