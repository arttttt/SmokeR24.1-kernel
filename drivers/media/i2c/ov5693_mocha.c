/*
 * ov5693_mocha.c - OV5693 sensor driver for Xiaomi Mi Pad 1 (mocha)
 *
 * Fork of ov5693.c with mocha-specific power sequence:
 * - 4 regulators + afvdd (5 total) instead of 2
 * - 3 GPIOs (pwdn, reset, af) instead of 2
 * - CSI-E DPD control
 * - BGGR Bayer format (not RGGB)
 *
...
/*
 * Mocha OV5693 power-on sequence from Xiaomi R21.5 board-ardbeg-sensors.c
 * ardbeg_ov5693_power_on():
 *
 *   1. CSI-E DPD off
 *   2. CAM2_PWDN=0, CAM2_RSTN=0, CAM_AF_PWDN=0
 *   3. usleep(10-20)
 *   4. afvdd ON  (imx179_reg1, LDO7, 2.7V)
 *   5. avdd ON   (avdd_ov5693, LDO4, 2.7V)
 *   6. vcmvdd ON (vdd_cam_1v8, fixed)
 *   7. CAM2_PWDN=1
 *   8. dvdd ON   (vdd_cam_1v2, fixed)
 *   9. udelay(2)
 *  10. CAM2_RSTN=1
 *  11. usleep(300-310)
 */
static int ov5693_power_on(struct camera_common_data *s_data)
{
	...
	if (pw->af_gpio)
		ov5693_gpio_set(priv, pw->af_gpio, 0);

	/* Step 3: settling time */
	usleep_range(10, 20);

	/* Step 4: afvdd ON (imx179_reg1, LDO7) */
	if (priv->afvdd) {
		err = regulator_enable(priv->afvdd);
		if (err)
	...
	/* Step 7: CAM2_PWDN high */
	if (pw->pwdn_gpio)
		ov5693_gpio_set(priv, pw->pwdn_gpio, 1);

	/* Step 8: dvdd ON (vdd_cam_1v2) */
	if (pw->dvdd) {
	...
	/* Step 9-10: settling + reset high */
	udelay(2);
	if (pw->reset_gpio)
		ov5693_gpio_set(priv, pw->reset_gpio, 1);

	/* Step 11: post-reset settling */
...
/*
 * Mocha OV5693 power-off sequence from Xiaomi R21.5 board-ardbeg-sensors.c
 * ardbeg_ov5693_power_off():
 *
 *   1. usleep(21-25)
 *   2. CAM2_RSTN=0
 *   3. udelay(2)
 *   4. dvdd OFF   (vdd_cam_1v2)
 *   5. vcmvdd OFF (vdd_cam_1v8)
 *   6. CAM2_PWDN=0
 *   7. CAM_AF_PWDN=0
 *   8. avdd OFF   (avdd_ov5693)
 *   9. afvdd OFF  (imx179_reg1)
 *  10. CSI-E DPD on
 */
...
static int ov5693_power_put(struct ov5693 *priv)
{
	...
	if (likely(priv->afvdd))
		regulator_put(priv->afvdd);
...
static int ov5693_power_get(struct ov5693 *priv)
{
	...
	/* digital core 1.2V (fixed regulator) */
	if (pdata->regulators.dvdd)
		err |= camera_common_regulator_get(priv->i2c_client,
				&pw->dvdd, pdata->regulators.dvdd);
	/* VCM 1.8V (fixed regulator) */
	if (pdata->regulators.vcmvdd)
		err |= camera_common_regulator_get(priv->i2c_client,
				&pw->vcmvdd, pdata->regulators.vcmvdd);
...
static u16 ov5693_to_real_gain(u32 rep, int shift)
{
	...
	/* derived from formula gain = (x * 16 + 0.5) */
...
static int ov5693_ctrls_init(struct ov5693 *priv, bool eeprom_ctrl)
{
	...
	for (i = 0; i < numctrls; i++) {
		/* Skip control 'V4L2_CID_EEPROM_DATA' if eeprom init err */
...
static struct camera_common_pdata *ov5693_parse_dt(struct i2c_client *client)
{
	...
	of_property_read_string(node, "vcmvdd-reg",
			...
			const struct i2c_device_id *id)
{
	...
	if (!err && afvdd_reg_name) {
		err = camera_common_regulator_get(client,
				&priv->afvdd, afvdd_reg_name);
		if (err)
			dev_warn(&client->dev,
	...
	dev_dbg(&client->dev, "Detected OV5693 sensor (mocha)\n");
