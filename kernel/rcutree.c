 ...
 * ->lock must be held by the caller, with irqs disabled. Returns true
 * if the grace-period kthread needs to be awakened.
 */
static bool
...
rcu_start_gp_per_cpu(struct rcu_state *rsp, struct rcu_node *rnp, struct rcu_data *rdp)
{
	...
	(void)__rcu_process_gp_end(rsp, rnp, rdp);
...
static void rcu_gp_cleanup(struct rcu_state *rsp)
{
	...
	rcu_for_each_node_breadth_first(rsp, rnp) {
		...
		rdp = this_cpu_ptr(rsp->rda);
		if (rnp == rdp->mynode)
			needgp = __rcu_process_gp_end(rsp, rnp, rdp) || needgp;
